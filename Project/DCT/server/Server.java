...
public enum Server {
     ...
     * @param serverThread   The client moving rooms
     * @return true if reassign worked; false if new room doesn't exist
     */
    protected synchronized boolean joinRoom(String roomName, DCT.server.ServerThread serverThread) {
        Room newRoom = roomName.equalsIgnoreCase(Constants.LOBBY) ? lobby : getRoom(roomName);
        Room oldRoom = serverThread.getCurrentRoom();
        if (newRoom != null && roomName != null) {
            if (oldRoom != null && oldRoom != newRoom) {
                logger.info(serverThread.getName() + " leaving room " + oldRoom.getName());
                oldRoom.removeClient(serverThread);
            }
            logger.info(serverThread.getName() + " joining room " + newRoom.getName());
            newRoom.addClient(serverThread);
            return true;
        } else {
            serverThread.sendMessage(Constants.DEFAULT_CLIENT_ID,
