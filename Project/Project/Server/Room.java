                ...
                switch (command) {
                    ...
                    case ROLL:
                        ...
                        if(diceRoll.contains("d")) {
                            int total = 0;
                            String dieCount = diceRoll; 
                            String[] comm3 = dieCount.split("d");
                            String numOfSides = comm3[1];
                            String numofDice = comm3[0];

                            for(int i=0; i<Integer.parseInt(numofDice);i++)
                            {
                                int randomNum = (int)(Math.random() * Integer.parseInt(numOfSides)) + 1;
                                sendMessage(client, "die " + (i + 1) + " rolled to number " + randomNum);
                                total += randomNum;
                            }
                            sendMessage(client, "total dice roll = " + total);

                        }
                        else{        
                            String[] comm3 = diceRoll.split("<i>"); 
                            sendMessage(client, diceRoll);
                            String maxNumber = comm3[1];
                            int randomNum = (int)(Math.random() * Integer.parseInt(maxNumber)) + 1;
                            sendMessage(client, "total dice roll = " + randomNum); 
                        }
                        break;

                    case FLIP:
                        Random randomObj = new Random();
                        int coinFlipper = randomObj.nextInt(2);
                        if(coinFlipper == 0)
                        {
